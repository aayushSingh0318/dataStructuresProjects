...
def groupNoAdj(start, nums, target):
    pos = nums[start]
    if start <= len(nums):
        return groupNoAdj(start + 2, nums, target-pos) or groupNoAdj(start + 1, nums, target)
    else:
        return (target == 0)
...
def groupSum5(start, nums, target):
    if start >= len(nums): #start point has to be less than the num of ele in array
        return target == 0
    elif nums[start] % 5 == 0:
        if start + 1 < len(nums) and nums[start + 1] == 1: #we do start+1 and its greater than array, we treat it as the last ele in array
...
def groupSumClump(start, nums, target):
    index = start +1
    if start <= len(nums):
        return groupSumClump(index, nums, target) or groupSumClump(index, nums, target - nums[start]) 
    elif (len(nums) > index) and (nums[index] == nums[start]):
        position = start
        while nums[start] == nums[position]:
            position = position + 1
        return groupSumClump(start + (position - start), nums, target) or groupSumClump(start + (position - start), nums, (target - (nums[position]*(position - start))))
    else:
        return (target == 0)
...
def splitArray(nums):
    ...
    sumFst = 0
    sumSec = 0
    return splitArrayHelper(start, nums, sumFst, sumSec)

def splitArrayHelper(start, nums, sumFst, sumSec):
    if start >= len(nums):
        return sumFst == sumSec
    else:
        return splitArrayHelper(start + 1, nums, sumFst + nums[start], sumSec) or splitArrayHelper(start + 1, nums, sumFst, sumSec + nums[start])
...
def splitOdd10(nums):
    ...
    group2 = 0
    origin = 0
    return splitOdd10Helper(nums, group1, group2, origin)
  
def splitOdd10Helper(nums, group1, group2, origin):
    if origin <= len(nums):
        return  splitOdd10Helper(nums, group1, group2 + nums[origin], origin + 1) or splitOdd10Helper(nums, origin + nums[group1], group2, origin + 1) 
    else:
...
def split53(nums):
    ...
    sumFst = 0
    sumSec = 0 
    return split53Helper(start, nums, sumFst, sumSec)

def split53Helper(start, nums, sumFst, sumSec):
    if start >= len(nums):
        return sumFst == sumSec
    else:
        if nums[start] % 5 == 0:
            return split53Helper(start + 1, nums, sumFst + nums[start], sumSec)
        elif nums[start] % 3 == 0:
            return split53Helper(start + 1, nums, sumFst, sumSec + nums[start])
        else:
            return split53Helper(start + 1, nums, sumFst + nums[start], sumSec) or split53Helper(start + 1, nums, sumFst, sumSec + nums[start])
